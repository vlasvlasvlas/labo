  ...
  
{
  ...
  for (ronda in rondas) {
    ...
    if ( GLOBAL_tiros_total + length(ids_juegan) < 14000 ) { #fix que los jugadores no tiren mas cuando lleguen al limite
      
      if ( length(ids_juegan) > 0) {
        
        
        # umbral de corte
        # 3 standard deviations from the expected value: n * pi - 3 * sqrt(n * pi * (1 - pi))        
        umbral <- ( (1:ronda) * 0.7 ) - 3.1999 * sqrt( (1:ronda) * 0.7 * (1 - 0.7) ) 
...
# -- seeds --------------------------------------------------------------------
set.seed( 825733 ) #debe ir una sola vez, ANTES de los experimentos
# primos
# 295873
# 527173
# 328789 
...
vb_nombreTotalmenteDiferente_15K_OK.R
