...
# lo que quise fue "parametrizar" lo que se podía cada ronda (% por encima de median o avg; multiplicador de tiradas x ronda, suma de tiradas x ronda, valor que se usa para un if cantidad de jugadores por encima de median o avg que quedaron es menor a dicho valor, desestima ronda)

# es cierto el efecto sabana corta se cumple en diferentes pruebas que realizé

# mi primera idea era "que generen pocos tiros al comienzo, y a medida que se achica el lote de jugadores, aumentar las jugadas con estos parámetros"

# pero ahora pienso "¿y si puedo generar rangos y trabajar de una forma similar a la que usamos los hiperparámetros donde se ajustan los parámetros?", "¿y si puedo una lógica diferente?" "¿y si puedo cambiar los parámetros de la corrida de estrategía de un forma diferente?

#     "y si debería hacer otra pregunta entre ronda y ronda para avanzar a la siguiente ronda o no?" <- por ejemplo, "¿cuantos tiros me quedan? teniendo en cuenta la cantidad de jugadores que me quedaron desde la ronda anterior para la presente ronda, y teniendo en cuenta los tiros que estoy a punto de hacerles a los jugadores, revisar si me paso de la suma total de tiros acumulada, para no continuar"

#     estoy buscando cual sería una mejor forma de preguntar si debo o no continuar con la siguiente ronda
...
median_ronda2     <- 1.0037  # probando uso el mean
...
tirada_inicial    <- 70     # vueltas en la primera ronda
mult_tiradas      <- 1.1    # se multiplican las tiradas por ronda en base al valor de tirada anterior
suma_tiradas      <- 0     # se suman a las tiradas por ronda por sobre el valor de ronda anterior
cant_experimentos <- 1000     # cantidad de experimentos, la achico por tiempos de demora y voy subiendolos a 10k
count_mayor_a     <- 2    # si el count length de ids dentro de la mediana anterior tiene al menos este valor, no ejecuta la nueva ronda
...
Estrategia_intento002  <- function()
